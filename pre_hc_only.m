...
hc = [1,2,4,7,8,10,18,20,21,24,25,29,30,31,32,33];
for jj=1:15
    disp(jj)
    ...
    hc = [1,2,4,7,8,10,18,20,21,24,25,29,30,31,32,33];
    pd = [3,5,6,9,11,12,13,14,16,17,19,22,23,26,28];
    data_num = hc(jj);
    ...
    if hc_pd == 0
        ...
        saved_name = ['nonEpoched_hc_',num2str(data_num),'_ready']; % hc1_epoched.set   OR   pd1_off_epoched.set
        EEG = eeg_checkset( EEG );
    elseif hc_pd == 1
    % for PDs::::::::::::
        data_num = pd(jj);
        ...
        saved_name = ['nonEpoched_pd_',num2str(data_num),'_off_ready']; % hc1_epoched.set   OR   pd1_off_epoched.set
        EEG = eeg_checkset( EEG );
    end
